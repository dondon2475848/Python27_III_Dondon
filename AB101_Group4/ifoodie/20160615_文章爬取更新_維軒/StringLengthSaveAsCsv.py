...
def createLog(listIndex,blog_id,blog_url,ExceptType):
    with open('./data/log/ArticleLenLess100_1_%s_%s_v12.txt' %(date,ExceptType) , 'a') as f:
        f.write(str(listIndex)+','+blog_id+','+blog_url+'\n')
def main():
    with open('./data/listAllArticle_New_0_72838_v12.json','r') as f:
    ...
    with open('./Excel/ArticleLenLess100_Report_v12.csv','wb') as f:
        w = csv.writer(f)
