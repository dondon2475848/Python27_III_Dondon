...
rs = requests.session()
rs.mount('https://', HTTPAdapter(max_retries=24)) #設定重試數量
...
def PrintKeyValue(dic_in):
    for key, value in dic_in.iteritems() :
        print key,'  :  ', value 
# 當愛食記內文章有問題，│抓取原始部落格的url
def ifoodieLinkToOriginBlog(urlIn):
    res=requests.get(urlIn,headers=head)
    res.encoding='utf-8'
    soup=bs(res.text,'lxml')
    return soup.findAll('a', href=True, target='_blank',rel='nofollow')[0].get('href') # 取得a標籤，且屬性target='_blank',rel='nofollow'
# 未使用，無法正確轉址，僅可連到首頁
...
def createLog(blog_id,blog_url,startIndex,endIndex,listIndex,ExceptType):
    with open('./data/log/%s_%d_%d.txt' % (ExceptType,startIndex,endIndex) , 'a') as f:
...
def deleteBadWords(StrIn):
    Str_BadWords = u'([0-9]|[０-９]|[ㄅ-ㄩ]|延伸閱讀|連絡方式|電話預約|電話|營業時間|週一|週二|週三|週四|週五|週六|週日|周一|周二|周三|周四|周五|周六|\
                    |周日|假日|公休|平日|地址|粉絲團|星期|禮拜|您或許對這些文章有興趣|造訪日期|全年無休|上一篇|下一篇|\
                    ...
                    |並不會被公開|你的位址 |迴響名稱|用餐日期|載入中|文章文章|粉絲頁|發表|每人平均價位|按個讚|推薦你閱讀|Instagram|instagram|\
...
#     print res.text
    ...
    ikachalife = soup.select('.post_content') #yam
    yam = soup.select('.articleBody') 
    ...
    elif urlType=='blog.yam.com': 
        art = ikachalife + yam
        time.sleep(3)   
    elif urlType=='www.wiselyview.cc': 
        art = wiselyview2
        #time.sleep(3)
...
def main():
    ...
    with open('./data/listAllArticle_New_0_72838_v11.json','r') as f:
        listAllArticle_Old = json.load(f)
    startIndex = 0                       # yamStart 13897
    endIndex   = len(listAllArticle_Old) # yamEnd   14570
    save_count =5000

    
    listAllArticle_New=copy.deepcopy(listAllArticle_Old) # 複製一份List準備跑迴圈
    ArticleLenLess100=0     # 文章內容少於100字的文章的Count
    GetArticleErrorCount=0  # GetArticleErrorCount的文章的Count
    OtherError=0            # OtherError的文章的Count
    listIndex=startIndex-1  # ListIndex
    count = 0               # 拿來計算幾筆存一次檔案

    for blog in listAllArticle_New[startIndex:endIndex]:
        count += 1
        listIndex += 1
        print listIndex,time.strftime('%Y%m%d_%H:%M:%S')
        # yam有問題，先不處理
#         uelTypeForYam= blog['url'].split('/')[2]
#         if uelTypeForYam=='blog.yam.com':                        
#             continue
        
        try:
            if blog['article']!=u'我沒有內容QQ':
                if len(blog['article']) < 100:
                    #blog.pop('article', None) 
                    ...
                    print blog['article']
                    print blog['url']
                    if len(blog['article'])<100 and blog['blog_type']=='ifoodie.tw' :
                        #blog.pop('article', None) 
                        blog['article'] = GetArticle(ifoodieLinkToOriginBlog(blog['url']))
                    if len(blog['article'])<100:
            ...
            try:
                ...
                createLog(blog['_id'],blog['url'],startIndex,endIndex,listIndex,'GetArticleError')
                GetArticleErrorCount+=1
        except: 
            createLog(blog['_id'],blog['url'],startIndex,endIndex,listIndex,'OtherError')
            OtherError+=1
        #每 save_count 筆存一次
#         if count%save_count==0:
#             print 'index為'+str(listIndex)+'時，Save file'
#             with open('./data/listAllArticle_New_%d_%d_%d.json' % (startIndex, endIndex,listIndex+1-startIndex), 'w') as f:
#                 json.dump(listAllArticle_New, f)
#         if listIndex==endIndex-1:
#             print 'index為'+str(listIndex)+'時，Save file'
#             #list全部拜訪完，存最後一次檔案。
#             with open('./data/listAllArticle_New_%d_%d_%d.json' % (startIndex, endIndex,endIndex-startIndex), 'w') as f:
#                         json.dump(listAllArticle_New, f)
        if listIndex==endIndex-1:
            print 'index為'+str(listIndex)+'時，Save file'
            #list全部拜訪完，存最後一次檔案。
            with open('./data/listAllArticle_New_0_72838_v12.json' , 'w') as f:
                        json.dump(listAllArticle_New, f)
    ...
    print '有',GetArticleErrorCount,'篇文章GetArticleErrorCount'
    print '有',OtherError,'篇文章 OtherError'
