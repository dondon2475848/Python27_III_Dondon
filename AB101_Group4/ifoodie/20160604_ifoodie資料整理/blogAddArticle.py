...
def main():
    ...
    with open('./data/dataOut/ifoodieBlogDic20160524AddAuthor_id.json' , 'r') as f:
        blogDic=json.load(f)
    with open('./data/listMerge_72838_13897_14568_v6.json' , 'r') as f:
    ...
    # 將文章內容為[]的資料令存成csv檔
    mkdir('./Excel/')  
    with open('./Excel/blogNoArticle.csv','wb') as f:
        w = csv.writer(f)
        w.writerows(blogNoArticleList)   
    with open('./data/dataOut/ifoodieBlogDic20160524Author_id_Article.json','w') as f:
        json.dump(blogDic,f)    
    endTime=time.time()
