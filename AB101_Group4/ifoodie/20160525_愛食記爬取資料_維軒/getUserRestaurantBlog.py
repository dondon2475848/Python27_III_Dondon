...
### 抓取的起點與結束點
beginIndex  = 12000        #起始點
endIndex    = 12800   #結束點,抓到這筆之前的資料
...
# userlist_select = ["552cf4632756dd722504d6ef"]   #橘子狗
...
for u in userlist_select:
    try:
        ...
        print "no response"
        createLog(u, 'user')   
    # 存檔分為兩部份，1. 1000為一筆  2.小於1000，但已將user_Id抓完，而存檔
    count+=1
    if count %save_count==0:               
        ...
        file_Name_Star=beginIndex+save_count*loop+1  # 檔名起始數字
        file_Nnme_End=beginIndex+count               # 檔名結束數字
        
        try:
            with open('./data/user/ifoodUsers_%d_%d.json' % (file_Name_Star, file_Nnme_End), 'w') as f:
                json.dump(user, f)
            with open('./data/restaurant/ifoodRestaurant_%d_%d.json' % (file_Name_Star, file_Nnme_End), 'w') as f:
                json.dump(restat, f)
            with open('./data/blog/ifoodBlog_%d_%d.json' % (file_Name_Star, file_Nnme_End), 'w') as f:
        ...
        file_Name_Star=beginIndex+save_count*loop+1  # 檔名起始數字
        file_Nnme_End=beginIndex+count-1               # 檔名結束數字
