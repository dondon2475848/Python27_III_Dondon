...
def main():
    ...
    for blog in listAllArticle[startIndex:endIndex]:
         blog['article'] = GetArticle(blog['url'])
         #print blog['article']
         articleList.append(blog)
         count+=1
         # 存檔分為兩部份，1. 1000為一筆  2.小於1000，但已將user_Id抓完，而存檔
         if count %save_count==0:
             file_Name_Star = startIndex+save_count*loop+1  # 檔名起始數字
             file_Nnme_End  = startIndex+count              # 檔名結束數字   
             
             with open('./data/blog/article/listArticle%d_%d.json' % (file_Name_Star, file_Nnme_End), 'w') as f:
                 json.dump(articleList, f)
             loop+=1
             articleList=[]
         elif blog['_id'] == listAllArticle[endIndex-1]['_id']: 
             file_Name_Star = startIndex+save_count*loop+1  # 檔名起始數字
             file_Nnme_End  = startIndex+count               # 檔名結束數字
             with open('./data/blog/article/listArticle%d_%d.json' % (file_Name_Star, file_Nnme_End), 'w') as f:
                 json.dump(articleList, f)
