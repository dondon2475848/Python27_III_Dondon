...
def main():
    ...
    with open('./data/blog/article/listMerge_72838_13897_14568_v6.json','r') as f:
        listAllArticle_Old = json.load(f)
    startIndex = 22056
    endIndex   = 22099
    save_count = 5000 #幾筆存一次
...
#                 json.dump(listAllArticle_New, f)
        if listIndex==endIndex-1:
            print str(listIndex)
            #list全部拜訪完，存最後一次檔案。
            with open('./data/blog/article/listAllArticle_New_%d_%d_%s.json' % (startIndex, endIndex,endIndex), 'w') as f:
                        json.dump(listAllArticle_New, f)
